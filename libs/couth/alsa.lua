-- MODDED BY KURE
-- My sound card is not the first (-c0), it's the second (-c1)
--
-- So, by replacing -c0 for -c1 on all the `amixer` commands,
-- everything went out just fine
--
-- Also changed the font on `couth.lua`
  ...
  for _,ctrl in ipairs(couth.CONFIG.ALSA_CONTROLS) do
    if volumes[ctrl] then
      ...
      if ctrl == ctrlToHighlight then
        prefix,suffix = '<span color="cyan">',"</span>"
      end
